...
describe("rời cuộc gọi thì dừng recorder", () => {
    test("clear() gọi leaveCall() của recorder TRƯỚC KHI dừng micAudioTrack gốc", () => {
        ...
        // KÊNH chứ không phải thành viên cuộc gọi. Gọi `leaveCall()`, KHÔNG
        // PHẢI `stop()` trơn: chỉ `leaveCall()` mới dọn luôn dấu "đã từ
        // chối", tránh nó rò rỉ sang một cuộc gọi khác join sau đó.
        //
        ...
        const recorder = {
            state: { recordingId: 7 },
            leaveCall() {
                fake.log.push("recorder.leaveCall");
        ...
        expect(fake.log).toEqual(["recorder.leaveCall", "exitFullscreen", "stopMic"]);
        expect(fake.state.micAudioTrack).toBe(undefined);
    });

    test("không đang ghi âm thì clear() không gọi leaveCall() lỗi", () => {
        // `MeetingRecorder.leaveCall()` gọi `stop()` bên trong, vốn đã tự
        // bảo vệ bằng guard riêng (recorder_service.js) — ở đây chỉ cần
        // chắc patch luôn gọi `leaveCall()`, và gọi trên một recorder không
        // ghi âm phải là no-op an toàn, không ném lỗi.
        const recorder = {
